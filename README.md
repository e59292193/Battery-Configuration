...
**计算器（工程正确性回归测试 + 直流专项测试，`npm test` 全绿）**

- 功率验证模式：UPS 需求 → 恒功率放电表查值 → 单组/总功率 → 功率满足率；恒定负荷同时显示同一查表口径的能量满足率（查表可用 Wh ÷ 需求 Wh）→ 备电时间 → 放电电流
- 能量验证模式（原容量验证）：需求 Wh → 恒功率表能量校核 → 能量满足率；Ah 视图按「恒功率表 ÷ 恒流表」的实际平均放电电压折算，仅供与铅酸方案对比
- 直流 UPS 模式：母线电压窗口定块数 → 恒流表（A/Cell）查值 → IEEE 485 分段法定组数 → 10C 脉冲校核；恒定负荷支持 A / kW 输入（kW 按放电下限 Vmin 折算电流）
- 预估备电时间反查放电表，相邻档位按 ln P – ln t 双对数插值（不做普通线性插值，杜绝凸曲线高估）
- 查表越界全量告警：EPV 高于最高档 / 低于最低档、时长超过最长档 / 短于最短档（5min 冲击段）
- 单组最大电流 vs 规格极限（10C）硬性校核、并联组数 >4 风险提示
- 智能配置推荐：EPV 只能取所选型号恒功率/恒流表的真实档位（禁止推荐 1.44V、1.38V 等无数据值），再在电压窗口可行域内搜索「总电芯数最少」的块数，窗口无解直接报错
- 阶梯工况下自动隐藏「要求的备电时间」（时长由工况表决定）；恒定负荷不显示分段满足率（单段三口径等价）
- 计算步骤详解位于左栏底部，与输入参数同侧对照
- EPV / 备电时间向上取有数据档位（禁止插值；推荐值必须精确命中表格行）、节数⇄块数双向联动、电压窗口校验仪表盘
...
- 每次请求自动注入：当前型号完整恒功率表 + 恒流表 + 全部输入与结果（含直流母线参数与两种阶梯工况表）+ 计算规则（页面底部可查看注入内容）；附件内容标注为不可信数据，防提示注入
- 历史上下文按消息截断（附件正文不重复回传），温度默认 0.2（确定性查表任务）
- 服务端可选加固：`API_AUTH_TOKEN` 访问口令、`MODEL_ALLOWLIST` 模型白名单、消息体积上限、模型列表按密钥指纹隔离缓存
...
| 功率法 | 查表（EPV、时间点向上取档，禁止插值）→ `功率满足率 = 总提供功率 ÷ 总需求功率`；恒定负荷另显示 `能量满足率 = (查表功率 × 时长) ÷ (需求功率 × 时长)`；备电时间由恒功率表**反查插值** `estimateRuntimeMin()`，越界以 `<` / `>` 标注 |
...
| 推荐器 | 推荐 EPV 必须精确存在于所选型号的表格行；块数同时受电压窗口约束：`maxBlocksByCharge`（充电上限）与 `minBlocksByDischarge`（放电下限）；无解时返回 `voltageWindowOk: false` |
| DC 系统 | `dcVoltMin` 是 UPS 最低允许母线值；电池推荐截止电压取恒流表真实档位且满足 `EPV × cells ≥ dcVoltMin`。能量统一按 13.2V 口径；浮充/均充电压由温补 TCV（1.9 / 1.85 V/cell）推导，超出母线窗口给 `chargeWithinBus` 告警；铅酸对比基准由需求能量推导（`VRLA_REF_WH_PER_KG=35`、`VRLA_REF_WH_PER_L=85`） |
...
回归测试：`npm test` 运行工程正确性黄金用例与直流专项测试（基准来自 HOPPECKE IEEE 485 选型报告，AEG 60kVA / 210kVA 工况），覆盖老化系数方向、分段选型控制断面、kVA 折算、反查备电时间、EPV 必须命中规格表、恒定负荷能量满足率、越界告警与电压窗口约束。
